...
/// <summary>
/// <see cref="TrackerPluginHost"/> against a real engine over a real pipe. The host is ~90 lines of
...
public class PluginHostIntegrationTests(ITestOutputHelper output)
{
    ...
    /// <see cref="ITrackerPlugin"/> runs to completion over the corpus without owning one line of
    ...
    public async Task WhenTheEngineRestarts_TheHostReconnectsAndFinishesTheRun()
    {
        ...
        var run = TrackerPluginHost.RunAsync(plugin, ["--pipe", pipeName], Options(hostOutput, cts.Token));
    ...
    public async Task WhenCancelledMidSession_ExitsZeroWithTheSummary()
    {
        ...
        var run = TrackerPluginHost.RunAsync(plugin, ["--pipe", pipeName],
