...
/// <c>CaptureEngine.Tests.PluginHostIntegrationTests</c>, the only project that can own both ends of
/// the pipe.
/// </summary>
public class TrackerPluginHostTests
{
    /// <summary>
    ...
    public async Task PipeFlagWithoutAValue_ExitsOneWithoutConnecting()
    {
        ...
        var exit = await TrackerPluginHost.RunAsync(plugin, ["--pipe"], Options(output));
    ...
    public async Task BlankPipeName_ExitsOne()
    {
        ...
        var exit = await TrackerPluginHost.RunAsync(new StubPlugin(), ["--pipe", "   "],
            Options(output));
    ...
    public async Task ExtraArgHandlerReturningAnError_ExitsOne()
    {
        ...
        var exit = await TrackerPluginHost.RunAsync(new StubPlugin(), [], options);
    ...
    public async Task ExtraArgHandlerReturningNull_LetsTheRunProceed()
    {
        ...
        var exit = await TrackerPluginHost.RunAsync(new StubPlugin(), ["--pipe", DeadPipe()], options)
    ...
    public async Task Cancellation_ExitsZeroAndStillPrintsTheSummary()
    {
        ...
        var exit = await TrackerPluginHost
    ...
    public async Task Banner_NamesThePluginAndThePipe()
    {
        ...
        Assert.Contains("=== Star Citizen Tracker — refinery ===", output.Lines);
    ...
    public async Task NullOptions_AreAccepted()
    {
        ...
        Assert.Equal(1, await TrackerPluginHost.RunAsync(new StubPlugin(), ["--pipe"]));
    }

    [Fact]
    public async Task NullPlugin_Throws()
        => await Assert.ThrowsAsync<ArgumentNullException>(
            () => TrackerPluginHost.RunAsync(null!, []));
