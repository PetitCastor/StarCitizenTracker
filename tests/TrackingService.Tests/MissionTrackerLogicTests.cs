...
public class MissionTrackerLogicTests
{
    [Theory]
    ...
    {
        var result = MissionTracker.ParseAcceptedCounter(tabText);
    ...
    public void ParseAcceptedCounter_NoMatch_ReturnsNull()
        => Assert.Null(MissionTracker.ParseAcceptedCounter("nothing relevant here"));
    ...
    public void IsNewMissionAccepted_OnlyTrueOnSimpleIncrement(int previous, int current, bool expected)
        => Assert.Equal(expected, MissionTracker.IsNewMissionAccepted(previous, current));
}
