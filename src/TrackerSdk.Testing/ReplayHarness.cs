...
/// <summary>
/// Runs a plugin against a real, spawned <c>CaptureEngine.exe</c> replaying a PNG corpus — the exact
...
public static class ReplayHarness
{
    ...
    public static async Task<ReplayResult> RunAsync(ReplayOptions o, CancellationToken ct = default)
    {
        ...
        var pipe = $"sctracker-test-{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            // The engine exits on its own once the corpus is exhausted (see CaptureEngine/Program.cs)
    ...
    /// <summary>Forwards every <see cref="ITrackerPlugin"/> member to <paramref name="inner"/>
    /// unchanged, except for capturing the reason a run ended — the one thing
    /// <see cref="TrackerPluginHost"/> tells the plugin but never hands back to its caller.</summary>
    private sealed class EndReasonRecorder(ITrackerPlugin inner) : ITrackerPlugin
    {
