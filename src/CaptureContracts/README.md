...
The wire contract between the Star Citizen Tracker capture engine and its plugins: the generated
...
the surface is reached through `SCTracker.Sdk`, which is what a plugin should be written against. A
...
```powershell
dotnet add package SCTracker.Contracts
```
