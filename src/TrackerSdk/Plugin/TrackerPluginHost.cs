...
/// </remarks>
public static class TrackerPluginHost
{
    /// <summary>
    ...
    /// simply restart.
    /// </returns>
    public static async Task<int> RunAsync(ITrackerPlugin plugin, string[] args,
    ...
    private static async Task<int> RunCoreAsync(ITrackerPlugin plugin, string[] args,
        PluginHostOptions options, IPluginOutput output)
    {
        output.WriteLine($"=== Star Citizen Tracker — {plugin.Name} ===");
    ...
    private static async Task<(StreamEndReason Reason, int ExitCode)> RunSessionsAsync(
        ITrackerPlugin plugin, CaptureClient client, PluginServices services, IPluginOutput output,
        PluginHostOptions options, string pipeName, CancellationToken ct)
    {
        ...
        while (true)
        {
            ...
            catch (TrackerSdkException) when (ct.IsCancellationRequested)
            {
    ...
    private static void WriteSummary(ITrackerPlugin plugin, List<TrackerRecord> records,
        IPluginOutput output)
    {
        output.WriteLine();
        output.WriteLine($"=== Summary: {records.Count} captures ===");
        foreach (var g in records.GroupBy(r => (r.Tracker, r.Trigger)))
            output.WriteLine($"  {g.Key.Tracker} ({g.Key.Trigger}): {g.Count()}");
